...
(function (window, document) {
	...
	Croppic = function (id, options) {
		...
		that.options = {
			...
			rotateControls: true,
			modal:false,
			customUploadButtonId:'upbutton_edit',
			loaderHtml:'',
			scaleToFill: true,
			processInline: false,
			loadPicture:'',
...
if($('div #news_title').text()=='') {
					console.log($('div #news_title').text());
					$('#alerts').html('Спершу введіть заголовок новини');
					return false;
				}else {				
				if (fileUploadId === "") {
			...
			that.form.find('input[type="file"]').change(function(){
				...
				if(that.options.processInline){			
					...
					try {
						...
						formData.append('factory_id', $('#fac_id').val());
						formData.append('news_title', $('#news_title').val());
						formData.append('news_anons', $('#news_anons').val());
					} catch(e) {
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
                ...
                that.destroy();
				$('#img_to_change').hide('fast');
				$('#cropContainerModal_edit').css('display', 'none');
                $('#outputImg_edit').before('<img class="croppedImg im" src="' + response.url + '">');
