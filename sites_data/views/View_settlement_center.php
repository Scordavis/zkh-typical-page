...
<link rel="stylesheet" type="text/css" href="/css_cab/normalize.css" />
		<link rel="stylesheet" type="text/css" href="/css_cab/vicons-font.css" />
		<link rel="stylesheet" type="text/css" href="/css_cab/base.css" />
		<link rel="stylesheet" type="text/css" href="/css_cab/buttons.css" />
...
$id_factory=$_GET['id'];
$site_factory=$_SERVER['HTTP_HOST'];//$_GET['site'];//
...
<style>
	...
	<link rel="stylesheet" type="text/css" href="/css_cab/css/request_form.css?b=1" />
<!--<div class="">-->
...
<div style="background:#ffa;width:100%;color:#000; text-align:center;font: 900 16px Ubuntu;">
 <form name="form33" action="/settlement_center//?id=<?=$id_factory?>&site=<?=$site_factory?>" method="POST" >
	...
	<span style="">
		...
		<!--<button class="button button--rayen button--border-medium button--text-thin button--size-l button--inverted" data-text="Send Message"><span>Send Message</span></button>style="text-align:center !important;"-->
		<button class="button button--rayen button--border-thin button--text-thick button--text-upper button--size-s button--inverted" data-text="Увійти" ><span>Увійти</span></button>
...
<a href="/settlement_center/addCabinet/?id=<?=$data['d']['id']?>&misto=<?=$data['d']['misto']?>&site=<?=$site_factory?>" >Зареєструватись
...
<div style="margin-left:0%;margin-bottom:20%;display:none;" id="remind">	
	<form name="form3" action="/settlement_center//?id=<?=$id_factory?>&site=<?=$site_factory?>" method="POST"  >
...
<header class="header" style="margin: 0;
   ...
   <!--<i class="fa-angle-down" style="color: #fff;text-size:12px;text-decoration: none;"></i>-->
      ...
      <ul>
		...
		<li><a href="/settlement_center/viewFlat/?id=<?=$res['id']?>&id_factory=<?=$id_factory?>" target="_blank">Відомості для нарахування</a></li>
...
<style type="text/css">
 ...
 <div class="div_user" style="border-top:#808497 3px solid ;border-bottom:#808497 3px solid ;padding-top:15px;padding-bottom:15px;">
   ...
   <div class="colleft">
       	<!--<form name="form3" action="/settlement_center/CabinetUser/?id=<?=$id_factory?>&site=<?=$site_factory?>" method="POST"  >-->
...
<table  id="voda" style="display:none;background:rgba(209, 210, 213,0.5);">
	...
	<?if($data['flat']['water_warm_oblic']=='1'){?><tr style="border-bottom:1px solid #fff;">
		...
		<td  align="right" style="border:none;font: 500 15px Ubuntu;">
			<span style="margin-right:20px;"><?=$last_counters['water_warm_oblic']?></span><?=$last_counters['d']?>-<?=$last_counters['m']?>-<?=$last_counters['y']?>
		</td>	
	</tr><?}?>
	<?if($data['flat']['water_oblic']=='1'){?><tr style="border-bottom:1px solid #fff;">
		...
		<td  align="right" style="border:none;font: 500 15px Ubuntu;">
			<span style="margin-right:20px;"><?=$last_counters['water_cold_oblic']?></span><?=$last_counters['d']?>-<?=$last_counters['m']?>-<?=$last_counters['y']?>
		</td>	
	</tr><?}?>
...
<!---->
 ...
 <span style="">
	...
	$.ajax({
		url: '/settlement_center/ModalStatistics/',
		data: {
...
<!--<a href="/settlement_center/addCabinet/?id=<?=$data['d']['id']?>&see&osob_rachunok=<?=$res['osob_rachunok']?>" target="_blank">Відомості з особового рахунку</a>-->
...
<section>
	...
	<section class="bng_section">
		<div class="cont">
			<div class="form">
			<form method="POST" name="offic" action="/settlement_center//?id=<?=$id_factory?>&site=<?=$site_factory?>">
...
function show_modalBlock(inc_file, window_width) {
	...
	$.ajax({
		url: '/settlement_center/ModalBlock/',
		data: {
			rachunok:'<?=$res['osob_rachunok']?>',
			factory_id:'<?=$id_factory?>',
			inc_file:inc_file,
...
function saveCounter(o_r, factory_id, posluga) {
	$('#preloader').fadeIn('300');
	$.ajax({
		url: '/settlement_center/saveTeploCounter/',
		data: {
			rachunok:'<?=$res['osob_rachunok']?>',
			factory_id:'<?=$id_factory?>',
...
function editi(id) {
		...
		.blur(function()
		{
			...
			$.ajax({	
						type: "POST",
						url: '/settlement_center/updatecellForm_3/',
